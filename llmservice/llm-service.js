...
async function sendIncorrectOptionsToLLM(model, question) {
  if (model === "empathy") {
    return await sendIncorrectOptionsToEmpathy(question);
  } else if (model === "gemini") {
    return await sendIncorrectOptionsToGemini(question);
  } else {
...
async function sendIncorrectOptionsToGemini(question) {
  try {
...
//It returns the answer to the question by using the LLM of empathy
async function sendIncorrectOptionsToEmpathy(question) {
  try {
...
app.post("/ask", async (req, res) => {
  try {
    ...
    if (model === "empathy") { //For using Empathy
      answer = await sendQuestionToEmpathy(prompt);
...
async function sendQuestionToEmpathy(question) {
  try {
    const config = llmConfigs['empathy'];
...
app.post("/generateIncorrectOptions", async (req, res) => {
  try {
    ...
    const { model, correctAnswer, type } = req.body;
    let question =
    ...
    const answer = await sendIncorrectOptionsToLLM(model, question);
